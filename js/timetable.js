...
(function () {
  ...
  w.v66SaveLessonEditor=function(day,pi,li,remove){
    ...
    activeTimetable.classes=buildClassBlocks(activeTimetable.slots); w.v66CloseLessonEditor(); refreshTimetableView();
  };
  ...
  w.v66SaveTimetable = async function () {
    ...
    try {
      ...
      const savedRow=saved?.data||saved; if(savedRow?.id){activeTimetableId=savedRow.id;activeTimetable={...activeTimetable,...savedRow};}
      if (w.showToast) showToast('Timetable draft saved', 'success');
      return true;
    } catch (e) { w.showToast ? showToast(e.message, 'error') : alert(e.message); return false; } finally { if (w.hideLoading) hideLoading(); }
  };
  ...
  w.v66PublishTimetable = async function () {
    ...
    try {
      const saved = await w.v66SaveTimetable();
      if (!saved) return;
      const scope = document.getElementById('ttScope')?.value || activeTimetable.scope || 'term'; const term = document.getElementById('ttTerm')?.value || activeTimetable.term || 'Term 1'; const year = Number(document.getElementById('ttYear')?.value || activeTimetable.year || new Date().getFullYear());
      ...
      w.showToast && showToast(published.message || `Timetable published for the ${scope}`, 'success');
      await w.showDashboardSection?.('timetable');
