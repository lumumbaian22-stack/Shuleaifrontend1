...
function checkTeacherAssignment(teacherId, classId, subject = '') {
    const tid = Number(teacherId || 0) || null;
    const cid = Number(classId || 0) || null;
    const sub = String(subject || '').trim().toLowerCase();
    const pools = [];
    try {
        const classLists = [window.allClasses, window.classes, window.__classes, window.__lastClasses].filter(Array.isArray);
        for (const list of classLists) {
            const cls = list.find(c => !cid || Number(c.id || c.classId) === cid);
            if (!cls) continue;
            if (!sub && tid && Number(cls.teacherId || cls.classTeacherId || cls.Teacher?.id) === tid) return true;
            ['subjectAssignments','TeacherSubjectAssignments','assignments','subjects'].forEach(k => { if (Array.isArray(cls[k])) pools.push(...cls[k]); });
        }
        [window.subjectAssignments, window.teacherSubjectAssignments, window.__subjectAssignments].filter(Array.isArray).forEach(list => pools.push(...list));
        return pools.some(a => {
            const aTeacher = Number(a.teacherId || a.TeacherId || a.Teacher?.id || 0) || null;
            const aClass = Number(a.classId || a.ClassId || a.Class?.id || 0) || null;
            const aSubject = String(a.subject || a.subjectName || a.name || '').trim().toLowerCase();
            const teacherOk = !tid || aTeacher === tid;
            const classOk = !cid || aClass === cid;
            const subjectOk = !sub || aSubject === sub;
            return teacherOk && classOk && subjectOk;
        });
    } catch (error) {
        console.warn('[Class Management] Could not check teacher assignment safely:', error);
        return false;
    }
}

// Helper used by older dashboard hooks and diagnostics.
// It must exist before the global export below; otherwise class-management.js
// crashes during script evaluation and the whole Classes section stops loading.
async function listAllTeachersAndClasses() {
    ...
    return { success: true, data: { classes, teachers }, classes, teachers };
}
