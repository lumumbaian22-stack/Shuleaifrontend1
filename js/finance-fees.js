...
(function(){
  ...
  function renderSummary(){ const t=totals(); return `<div class="finance-v31-summary">
    ...
    <div class="finance-v31-metric"><div><strong style="color:var(--ff-blue)">Parent Paid</strong><h3>${money(t.parentPaid || 0)}</h3><small>Credits/Bursaries: ${money(t.credits || 0)}</small></div><div class="finance-v31-icon blue">↓</div></div>
  ...
  function renderSettings(){
    ...
    return `<div class="finance-v31-body"><div id="finance-v31-message" class="finance-v31-message"></div>
      ...
      <div class="finance-v31-settings-grid">
        <div class="finance-v31-form-card spacious"><h3>Payment Mode</h3>
          <label>Collection Mode</label><select id="finance-payment-mode" class="finance-v31-select">
          ...
          <div class="finance-v31-notice">Manual and automatic methods can run together without interfering with school fee tracking. Balances reduce only after approved/successful transactions.</div>
        </div>
        <div class="finance-v31-form-card spacious"><h3>Manual M-Pesa Public Details</h3>
          <label>M-Pesa Type</label><select id="finance-mpesa-type" class="finance-v31-select"><option value="paybill" ${s.mpesaType!=='till'?'selected':''}>Paybill</option><option value="till" ${s.mpesaType==='till'?'selected':''}>Till</option></select>
          <label>Paybill / Till Number</label><input id="finance-paybill" class="finance-v31-input" value="${esc(s.paybill||s.businessShortcode||s.shortcode||s.till||'')}" placeholder="e.g. 400200 or school till">
          <label>Account Reference Format</label><select id="finance-reference" class="finance-v31-select"><option value="elimuid" ${ref==='elimuid'?'selected':''}>Elimu ID</option><option value="admissionNumber" ${ref==='admissionNumber'?'selected':''}>Student Admission Number</option><option value="studentId" ${ref==='studentId'?'selected':''}>Student ID</option><option value="term" ${ref==='term'?'selected':''}>Student + Term</option></select>
          <label>Parent Instructions</label><textarea id="finance-manual-instructions" class="finance-v31-input" rows="4" placeholder="Example: Pay to the school paybill then submit the M-Pesa code for verification.">${esc(s.manualInstructions||s.metadata?.manualInstructions||'Pay using the displayed school account details, then submit your payment reference for verification.')}</textarea>
        </div>
        <div class="finance-v31-form-card spacious"><h3>Daraja STK Private Credentials</h3>
          <label>Daraja Consumer Key</label><input id="finance-daraja-key" class="finance-v31-input" placeholder="Private: only for Daraja mode" autocomplete="off">
          <label>Daraja Consumer Secret</label><input id="finance-daraja-secret" class="finance-v31-input" placeholder="Private: only for Daraja mode" autocomplete="off">
          <label>Daraja Passkey</label><input id="finance-daraja-passkey" class="finance-v31-input" placeholder="Private: only for Daraja mode" autocomplete="off">
          <label>Daraja Shortcode</label><input id="finance-daraja-shortcode" class="finance-v31-input" value="${esc(s.shortcode||s.businessShortcode||'')}" placeholder="Shortcode used for STK">
          <div class="finance-v31-notice">Parents never see these secret credentials. They only see the STK Pay button when Daraja is enabled and configured.</div>
        </div>
        <div class="finance-v31-form-card spacious"><h3>Bank / Cash / Card Details</h3>
          <label>Bank Name</label><input id="finance-bank-name" class="finance-v31-input" placeholder="Bank Name" value="${esc(b.bankName||s.bankName||'')}">
          <label>Account Name</label><input id="finance-account-name" class="finance-v31-input" placeholder="Account Name" value="${esc(b.accountName||s.accountName||'')}">
          <label>Account Number</label><input id="finance-account-number" class="finance-v31-input" placeholder="Account Number" value="${esc(b.accountNumber||s.bankAccount||s.accountNumber||'')}">
          <label>Branch</label><input id="finance-branch" class="finance-v31-input" placeholder="Branch" value="${esc(b.branch||s.branch||'')}">
          <label class="finance-v31-check"><input id="finance-cash-enabled" type="checkbox" ${cashEnabled?'checked':''}> Cash accepted at school office</label>
          <label class="finance-v31-check"><input id="finance-card-enabled" type="checkbox" ${cardEnabled?'checked':''}> Card/POS accepted at school office</label>
        ...
        <div class="finance-v31-total-box"><strong>Parent Visibility</strong><p style="color:var(--ff-muted)">Parents will see manual M-Pesa, STK availability, bank, cash and card instructions depending on what you enable. Daraja keys are never returned to parent dashboards.</p><button class="finance-v31-btn blue" onclick="financeV31TestConnection()">Test Daraja Connection</button></div>
      </div></div>`;
  }
  ...
  function renderRecords(){
    ...
    const accounts = rows.length ? rows : summary.accounts;
  ...
  function studentOptions(selectedId=''){
    const seen=new Set();
    return (state.paymentRecords||state.accounts||[]).map(a=>({id:a.studentId, name:a.studentName||accountStudentName(a), className:a.className||accountClassName(a)})).filter(a=>a.id && !seen.has(String(a.id)) && seen.add(String(a.id))).map(a=>`<option value="${esc(a.id)}" ${String(selectedId)===String(a.id)?'selected':''}>${esc(a.name)} • ${esc(a.className||'')}</option>`).join('');
  }
  function feeOptions(studentId='', selectedFeeId=''){
    return (state.paymentRecords||state.accounts||[]).filter(a=>!studentId || String(a.studentId)===String(studentId)).map(a=>({id:a.feeId||a.id, label:`${a.term||a.feeTerm||'Fee'} ${a.year||a.feeYear||''} • Balance ${money(a.feeBalance??a.balance??0)}`})).filter(a=>a.id).map(a=>`<option value="${esc(a.id)}" ${String(selectedFeeId)===String(a.id)?'selected':''}>${esc(a.label)}</option>`).join('');
  }
