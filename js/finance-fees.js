...
(function(){
  ...
  const state={tab:'overview',structures:[],classes:[],students:[],financeStaff:[],settings:{},accounts:[],paymentRecords:[],manualQueue:[],overview:null,expenses:[],financeAlerts:[],invoices:[],financeAnalytics:null,auditTrail:[],paymentLoadError:'',loading:false,filters:{className:'',term:'',year:String(new Date().getFullYear())}};
  ...
  function canManageFinanceStaff(){return currentRole()==='admin'||currentRole()==='super_admin'||currentRole()==='superadmin';}function isSchoolAdminView(){return currentRole()==='admin';}function isFinanceStaffView(){return currentRole()==='finance_officer';}function financeAllowed(p){const u=currentUser(),c=u?.financePermissions||u?.preferences?.finance?.permissions||[];return!isFinanceStaffView()||!Array.isArray(c)||!c.length||c.includes(p);}function financeStaffTitle(){const u=currentUser();return u?.financeTitle||u?.preferences?.finance?.title||state?.overview?.permissions?.financeTitle||'Finance Officer';}const FINANCE_ROLE_PRESETS={"Finance Officer":['overview','fee_structures','invoices','payments','verification','balances','expenses','alerts','analytics','reports','settings','audit'],Bursar:['overview','fee_structures','invoices','payments','verification','balances','reports','alerts'],Accountant:['overview','payments','verification','expenses','analytics','reports','audit']};
