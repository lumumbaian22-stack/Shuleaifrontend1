...
(function(){
  ...
  const FINANCE_PERMS={overview:'Overview',fee_structures:'Fee Structures',payments:'Payments/Receipts',verification:'Verification/Reconciliation',balances:'Balances/Defaulters/Bursaries',expenses:'Expenses',reports:'Reports',settings:'Settings',alerts:'Finance Alerts'};
  const state={tab:'overview',structures:[],classes:[],students:[],financeStaff:[],settings:{},accounts:[],paymentRecords:[],manualQueue:[],overview:null,expenses:[],financeAlerts:[],loading:false,filters:{className:'',term:'',year:String(new Date().getFullYear())}};
  ...
  function visibleClasses(){
    ...
    (state.students||[]).forEach(s=>names.add(s.className || s.Class?.name || s.grade || s.class || s.stream));
  ...
  async function loadOverview(){state.overview=await call(()=>apiSafe().finance.getOverview({year:state.filters.year||undefined,term:state.filters.term||undefined}),null);}
  ...
  function renderTabs(){if(isSchoolAdminView())return`<div class="finance-v31-tabs"><div class="finance-v31-tab ${state.tab==='overview'?'active':''}" onclick="financeV31SetTab('overview')">⌂ Finance Overview</div><div class="finance-v31-tab ${state.tab==='team'?'active':''}" onclick="financeV31SetTab('team')">👥 Finance Team</div></div>`;const tab=(perm,key,label)=>financeAllowed(perm)?`<div class="finance-v31-tab ${state.tab===key?'active':''}" onclick="financeV31SetTab('${key}')">${label}</div>`:'';return`<div class="finance-v31-tabs">${tab('overview','overview','⌂ Overview')}${tab('fee_structures','structures','▦ Fee Structures')}${tab('payments','records','▥ Payments & Receipts')}${tab('balances','balances','◫ Balances & Bursaries')}${tab('verification','verification','✓ Verification & Reconciliation')}${tab('expenses','expenses','↗ Expenses')}${tab('alerts','finance-alerts','🔔 Alerts')}${tab('reports','reports','▤ Reports')}${tab('settings','settings','⚙ Settings')}</div>`;}
  ...
  function body(){if(state.tab==='overview')return renderFinanceOverview();if(state.tab==='team')return renderFinanceTeam();if(state.tab==='settings')return renderSettings();if(state.tab==='records'||state.tab==='balances')return renderRecords();if(state.tab==='defaulters')return renderDefaulters();if(state.tab==='verification')return renderVerification();if(state.tab==='expenses')return renderExpenses();if(state.tab==='finance-alerts')return renderFinanceAlerts();if(state.tab==='reports')return renderReports();return renderStructures();}
  async function render(){if(blockNonAdminFinance())return'<div class="rounded-xl border bg-card p-6 text-red-600">Finance access is not assigned to this account.</div>';if(isSchoolAdminView()&&!['overview','team'].includes(state.tab))state.tab='overview';await loadAll();const admin=isSchoolAdminView();return`<section class="finance-v31 space-y-5"><div class="flex flex-col gap-3 rounded-2xl border bg-card p-6 md:flex-row md:items-center md:justify-between"><div><p class="text-xs font-semibold uppercase tracking-[0.18em] text-primary">School Finance</p><h1 class="mt-1 text-2xl font-bold">${admin?'Finance Overview':'Finance Workspace'}</h1><p class="mt-1 text-sm text-muted-foreground">${admin?'School totals, defaulters and Finance Team management.':'Fees, confirmations, balances, receipts, expenses and reconciliation for this school only.'}</p></div><div class="flex gap-2"><button class="finance-v31-btn" onclick="financeV31Refresh()">Refresh</button>${admin?'':`<button class="finance-v31-btn primary" onclick="financeV31OpenStructureModal()">+ Create Fee Structure</button>`}</div></div><div id="finance-v31-summary-wrap">${renderSummary()}</div><div class="finance-v31-shell">${renderTabs()}<div id="finance-v31-tab-body">${body()}</div></div></section>`;}
  ...
  w.financeV148RemoveStaff=async function(id){if(!confirm('Remove Finance Team access?'))return;try{await apiSafe().admin.updateFinanceStaff(id,{removeFinanceRole:true});await loadFinanceStaff();rerenderBody();showToast('Finance role removed','success');}catch(e){showToast(e.message||'Could not remove role','error');}};
