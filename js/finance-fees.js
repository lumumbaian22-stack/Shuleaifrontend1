...
(function(){
  ...
  const PAYMENT_AGENT_DEFS = [
    { provider:'mpesa', label:'M-Pesa', description:'Native Safaricom Daraja STK Push for parent fees.', fields:[['environment','Environment / Mode','sandbox or production'],['consumerKey','Consumer Key','',true],['consumerSecret','Consumer Secret','',true],['passkey','Passkey','',true],['shortcode','Shortcode','']] },
    { provider:'pesapal', label:'Pesapal', description:'Secure hosted checkout with automatic PesaPal IPN setup and verification.', fields:[['environment','Environment / Mode','sandbox or production'],['consumerKey','Consumer Key','',true],['consumerSecret','Consumer Secret','',true],['ipnId','IPN ID / notification_id','auto-filled after setup']] },
  ...
  async function saveProviderAgentDraft(provider, { feedback = false, refresh = false } = {}) {
  ...
  w.financeV31SaveProviderAgent = async function(provider){
    clearMessage();
    try { await saveProviderAgentDraft(provider, { feedback: true, refresh: true }); }
    catch(e){ setMessage('error', e.message || 'Could not save payment agent.'); }
  };

  w.financeV31TestConnection = async function(provider){ clearMessage(); try{ await saveProviderAgentDraft(provider); const res = await (apiSafe().payments?.testSchoolConnection ? apiSafe().payments.testSchoolConnection() : apiRequest('/api/payments/admin/test-connection',{method:'POST'})); setMessage('success', res?.message || 'Provider connection verified successfully.'); await loadSettings(); renderBodyOnly(); }catch(e){ setMessage('error', e.message || 'Provider connection test failed.'); } };

  w.financeV31TestStk = async function(provider){
    clearMessage();
    const hosted = ['stripe','pesapal'].includes(String(provider||'').toLowerCase());
    const phone = hosted ? '' : prompt('Enter admin/finance test phone for KSh 1 provider prompt. This test will NOT update student balances.', '2547');
    if(!hosted && !phone) return;
    try{
      await saveProviderAgentDraft(provider);
      ...
      setMessage('success', res?.message || (hosted ? 'Secure checkout test created.' : 'Provider prompt test started. Check the phone.'));
