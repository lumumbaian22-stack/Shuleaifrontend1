...
function updateUserInfo() {
    ...
    const photo = user.profileImage || user.profilePicture || user.avatar || '';
    ...
    if (avatarWrap) {
        const src = photo && typeof resolveMediaUrl === 'function' ? resolveMediaUrl(photo) : photo;
        if (src) {
            avatarWrap.className = 'h-8 w-8 rounded-full overflow-hidden flex items-center justify-center bg-muted';
            ...
            avatarWrap.className = 'h-8 w-8 rounded-full bg-gradient-to-br from-blue-500 to-violet-600 flex items-center justify-center text-white font-medium text-sm';
