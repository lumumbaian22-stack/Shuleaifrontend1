...
function hasSchoolFeature(feature) {
    ...
    // School plans are now size-based only. Every active paying/pilot/demo school receives
    // the complete core platform. Role, school structure, assignments and ownership still
    // control what each user can see. Suspended schools retain billing/settings access only.
    const suspended = school.suspended || school.access?.suspended || String(school.accessMode || school.access?.accessMode || '').toLowerCase() === 'suspended';
    if (suspended) return ['billing','school_settings'].includes(feature);
    return true;
}
    ...
    const sidebarConfig = {
        ...
        admin: {
            main: [
                ...
                { icon: 'cake', label: 'School Birthdays', section: 'birthdays' },
...
function dashboardSectionTitle(role, section) {
    ...
    const shared = {
        ...
        'report-comments-admin': 'Review Report Comments',
...
async function showDashboardSection(section) {
    ...
    try {
        const sectionNames = {
            ...
            'report-settings': 'Report Card Settings',
            'report-comments-admin': 'Review Report Comments',
            ...
            birthdays: currentRole === 'admin' ? 'School Birthdays' : 'My Class Birthdays',
...
async function renderDashboardSection(role, section) {
    ...
    if (section === 'alerts') {
        const renderer = window.v12RenderAlertsCenter || window.renderAlertsCenter;
        return typeof renderer === 'function'
            ? await renderer(role)
            : '<div class="rounded-xl border bg-card p-6"><h2 class="text-xl font-semibold">Alerts</h2><p class="mt-2 text-muted-foreground">Alerts are loading. Please reopen this section.</p></div>';
    }
    switch(role) {
        case 'superadmin':
            if (section === 'analytics') {
