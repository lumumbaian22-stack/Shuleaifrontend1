...
(function(){
  ...
  function subjectsText(t){ const a=t?.TeacherSubjectAssignments||[]; const rows=a.filter(x=>!x?.isClassTeacher).map(x=>`${x?.Class?.name||x?.Class?.grade||'Class'} — ${x?.subject||'Subject'}`); return rows.length?rows.join(', '):(Array.isArray(t?.subjects)?t.subjects.join(', '):(t?.subjects||'')); }
  function classTeacherText(t){ const a=(t?.TeacherSubjectAssignments||[]).find(x=>x?.isClassTeacher)?.Class; const c=a||(t?.Classes||[])[0]||t?.Class; return c?.name||c?.grade||'Not assigned'; }
  ...
  window.editTeacher = async function(id){
    try{
      ...
      if(!t) throw new Error('Teacher not found');
      const p = teacherProfile(t), name = teacherName(t);
      const assignmentSummary = `<div class="approved-field approved-span-2"><label>Current Class & Subject Assignments</label><div class="rounded-lg border bg-muted/30 p-3 text-sm"><div><strong>Class Teacher:</strong> ${esc(classTeacherText(t))}</div><div class="mt-1"><strong>Subjects:</strong> ${esc(subjectsText(t)||'Not assigned')}</div><button type="button" class="approved-btn mt-3" onclick="document.getElementById('approved-teacher-edit')?.remove(); openTeacherAssignments('${id}')">Manage Assignments in Classes</button></div></div>`;
      const body = `<div class="approved-profile-row">${avatar(name, teacherPhoto(t))}<div><h2 class="approved-name">Edit Teacher</h2><div class="approved-subtitle">Update professional information. Class and subject assignments are managed separately.</div></div></div><div class="approved-form-grid"><div class="approved-field"><label>Full Name</label><input id="approved-teacher-name" value="${esc(name)}"></div><div class="approved-field"><label>Teacher ID</label><input id="approved-teacher-employee" value="${esc(t.employeeId || '')}"></div><div class="approved-field"><label>TSC Number</label><input id="approved-teacher-tsc" value="${esc(p.tscNumber || t.tscNumber || '')}"></div><div class="approved-field"><label>Phone Number</label><input id="approved-teacher-phone" value="${esc(teacherPhone(t))}"></div><div class="approved-field"><label>Email Address</label><input type="email" id="approved-teacher-email" value="${esc(teacherEmail(t))}"></div><div class="approved-field"><label>Department</label><input id="approved-teacher-department" value="${esc(t.department || '')}"></div><div class="approved-field"><label>Qualification</label><input id="approved-teacher-qualification" value="${esc(t.qualification || '')}"></div><div class="approved-field"><label>Specialization / Main Subject</label><input id="approved-teacher-specialization" value="${esc(t.specialization || '')}"></div>${assignmentSummary}<div class="approved-field"><label>Status</label><select id="approved-teacher-status"><option value="approved" ${t.approvalStatus==='approved'?'selected':''}>Active</option><option value="pending" ${t.approvalStatus==='pending'?'selected':''}>Pending</option><option value="suspended" ${t.approvalStatus==='suspended'?'selected':''}>Suspended</option><option value="rejected" ${t.approvalStatus==='rejected'?'selected':''}>Rejected</option></select></div><div class="approved-field approved-span-2"><label>Notes</label><textarea id="approved-teacher-notes">${esc(p.notes || '')}</textarea></div></div>`;
      modal('approved-teacher-edit','Edit Teacher','Professional details only',body,`<button class="approved-btn" onclick="document.getElementById('approved-teacher-edit')?.remove()">Cancel</button><button class="approved-btn primary" onclick="approvedSaveTeacher('${id}')">Save Changes</button>`,true);
