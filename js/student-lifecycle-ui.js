...
(function(){
  ...
  function studentOptionsForClass(students, classId=''){
    return `<option value="">${classId?'Select student in this class':'Select class first'}</option>`+(students||[]).filter(s=>!classId||String(s.classId)===String(classId)).map(s=>`<option value="${s.id}" data-class-id="${s.classId||''}">${esc(studentLabel(s))} · ${esc(s.elimuid||'No Elimu ID')}</option>`).join('');
  }
  ...
  async function renderClassTransferCentre(role='admin'){
    ...
    return `<div class="space-y-6 animate-fade-in">
      ...
      <section class="rounded-2xl border bg-card p-5"><div class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <label class="text-sm">Target class<select id="class-transfer-target" class="mt-1 w-full rounded-lg border bg-background px-3 py-2"><option value="">Select target class</option>${groupedClassOptions(classes)}</select></label>
  ...
  function updateTransferCurrentClass(){const studentId=Number(document.getElementById('class-transfer-student')?.value||0);const student=transferState.options.students.find(s=>Number(s.id)===studentId);const field=document.getElementById('class-transfer-current');if(field)field.value=student?classLabel(student.classId):'Select a student';const target=document.getElementById('class-transfer-target');if(target&&student)for(const option of target.options)option.disabled=Number(option.value)===Number(student.classId);}

  async function previewStudentClassTransfer(role){
    const payload=readTransferForm();if(!payload.studentId||!payload.toClassId||!payload.effectiveDate||!payload.academicYear||!payload.term||!payload.reason)return showToast('Complete the student, target class, date, year, term and reason.','error');
    showLoading();try{const response=await roleApi(role).previewClassTransfer(payload);const preview=unwrap(response);transferState.preview={...payload,...preview};const panel=document.getElementById('class-transfer-preview');const fee=preview.feePreview||{},impact=preview.impactPreview||{};panel.innerHTML=`<section class="rounded-2xl border bg-card p-5"><h3 class="font-semibold text-lg">Transfer review</h3><div class="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-4"><div class="rounded-xl bg-muted/40 p-4"><p class="text-xs text-muted-foreground">Student</p><p class="font-semibold">${esc(preview.student?.name)}</p><p class="text-xs">${esc(preview.student?.elimuid||'')}</p></div><div class="rounded-xl bg-muted/40 p-4"><p class="text-xs text-muted-foreground">Movement</p><p class="font-semibold">${esc(preview.fromClass?.name)} → ${esc(preview.toClass?.name)}</p></div><div class="rounded-xl bg-muted/40 p-4"><p class="text-xs text-muted-foreground">Effective</p><p class="font-semibold">${esc(preview.effectiveDate)}</p><p class="text-xs">${esc(preview.term)} · ${esc(preview.academicYear)}</p></div><div class="rounded-xl bg-muted/40 p-4"><p class="text-xs text-muted-foreground">Fee difference</p><p class="font-semibold">KES ${Number(fee.difference||0).toLocaleString()}</p><p class="text-xs">${esc(fee.message||'Existing fees remain unchanged.')}</p></div></div>${impact.requiresAcknowledgement?`<label class="mt-4 flex gap-2 rounded-xl border border-amber-300 bg-amber-50 p-4 text-sm text-amber-900"><input id="class-transfer-impact-ack" type="checkbox" class="mt-1"><span>I acknowledge that this backdated transfer touches ${impact.attendanceCount||0} attendance record(s), ${impact.academicRecordCount||0} academic record(s), and ${impact.publishedReportCount||0} published report(s). These records will remain under the old class and will not be rewritten.</span></label>`:''}<div class="mt-4 rounded-xl border p-4 text-sm"><strong>History protection:</strong> The current enrollment will close on the day before ${esc(preview.effectiveDate)}. A new active enrollment starts on ${esc(preview.effectiveDate)}. Class teachers, marks, reports, payments and attendance already recorded remain unchanged.</div><div class="mt-4 flex justify-end"><button onclick="submitStudentClassTransfer('${role}')" class="px-5 py-2.5 rounded-lg bg-primary text-primary-foreground">${role==='teacher'?'Send for Admin Approval':'Confirm Transfer'}</button></div></section>`;panel.scrollIntoView({behavior:'smooth',block:'start'});}catch(error){showToast(error.message||'Transfer preview failed','error');}finally{hideLoading();}
  }

  async function submitStudentClassTransfer(role){const payload={...readTransferForm(),acknowledgeHistoricalImpact:!!document.getElementById('class-transfer-impact-ack')?.checked};if(transferState.preview?.impactPreview?.requiresAcknowledgement&&!payload.acknowledgeHistoricalImpact)return showToast('Confirm the historical-record acknowledgement before continuing.','error');if(!confirm(role==='teacher'?'Send this transfer request to the school administrator?':'Confirm this student class transfer?'))return;showLoading();try{const response=role==='teacher'?await api.teacher.requestClassTransfer(payload):await api.admin.createClassTransfer(payload);{ const st=transferState.options.students.find(s=>Number(s.id)===Number(payload.studentId)); showToast(response.message||`${studentLabel(st)} moved from ${classLabel(st?.classId)} to ${classLabel(payload.toClassId)}.`, 'success'); }await refreshClassTransferCentre(role);}catch(error){showToast(error.message||'Class transfer failed','error');}finally{hideLoading();}}
