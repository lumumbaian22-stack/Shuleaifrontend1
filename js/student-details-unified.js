...
async function printReportCard() {
    ...
    const studentId = currentStudentData.id || currentStudentData.studentId || currentStudentData.student?.id;
    if (!studentId) {
    ...
    try {
        ...
        const rows = Array.isArray(response?.data) ? response.data : (Array.isArray(response?.reports) ? response.reports : []);
        const latest = rows.find(r => r.isCurrent !== false) || rows[0];
        if (!latest?.id) {
            showToast('No published report card yet. Publish the class report first.', 'warning');
            return;
        }
        if (typeof openReportSnapshotPdf === 'function') await openReportSnapshotPdf(latest.id);
        else window.open(`${API_BASE_URL}/api/report-cards/history/${encodeURIComponent(latest.id)}/pdf`, '_blank', 'noopener');
    } catch (error) {
        showToast(error.message || 'Official report PDF could not be opened.', 'error');
    } finally {
