...
// v145: complete teacher lifecycle actions used by the table buttons.
window.deactivateTeacher = async function(teacherId, teacherName) {
  const reason = window.prompt ? window.prompt(`Reason for deactivating ${teacherName || 'this teacher'}:`, 'Temporarily inactive') : 'Temporarily inactive';
  if (reason === null) return;
  try { showLoading?.(); await api.admin.deactivateTeacher(teacherId, { reason }); showToast?.('Teacher deactivated', 'success'); await refreshTeachersList(); }
  catch (error) { showToast?.(error.message || 'Could not deactivate teacher', 'error'); }
  finally { hideLoading?.(); }
};
window.activateTeacher = async function(teacherId) {
  try { showLoading?.(); await api.admin.activateTeacher(teacherId); showToast?.('Teacher activated', 'success'); await refreshTeachersList(); }
  catch (error) { showToast?.(error.message || 'Could not activate teacher', 'error'); }
  finally { hideLoading?.(); }
};
