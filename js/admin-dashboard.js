...
async function renderAdminFairnessReport() {
    ...
    try {
        ...
        const hasData = fairnessData.summary?.hasDutyData === true;
        return `
            <div class="space-y-6 animate-fade-in">
...
async function saveAdminReportSettings() {
    ...
    const countedWeight = assessmentSettings
      .filter(setting => setting.isActive && setting.countInFinal)
      .reduce((sum, setting) => sum + Number(setting.weight || 0), 0);
    if (Math.abs(countedWeight - 100) > 0.01) {
      showToast(`Counted assessment weights must total 100%. Current total: ${countedWeight}%`, 'error');
      return;
    }
...
async function renderAdminReportComments() {
  let rows = [];
  try {
    const res = await api.admin.getStudents({ page: 1, limit: 200 });
                    ...
                    <div class="rounded-xl border bg-card p-5 flex flex-col ${activePlanCode === plan.code ? 'ring-2 ring-primary' : ''}">
                        <div class="flex items-start justify-between gap-3">
                            <div>
                                ...
                                <p class="text-sm text-muted-foreground">${Number(plan.minStudents ?? plan.limits?.minStudents ?? 1).toLocaleString()}${(plan.maxStudents ?? plan.limits?.maxStudents) == null ? '+' : `–${Number(plan.maxStudents ?? plan.limits?.maxStudents).toLocaleString()}`} active students</p>
                            </div>
