...
window.suspendTeacher = async function(teacherId, teacherName) {
    ...
    showLoading();
    try {
        // Note: Your backend may not have suspendTeacher endpoint; fallback to deactivate user
                ...
                <div class="rounded-xl border bg-card p-6">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="text" id="settings-school-name" value="${schoolSettings.name || schoolSettings.schoolName || ''}" class="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm">
                        </div>
                        <div>
...
window.viewTeacherDetails = async function(teacherId) {
    ...
    content.innerHTML = `
        <div class="flex items-center gap-4 pb-4 border-b">
            ...
            <div>
                <p class="text-lg font-semibold">${escapeHtml(teacher.User?.name)}</p>
                <p class="text-sm text-muted-foreground">${escapeHtml(teacher.User?.email)}</p>
            </div>
        </div>
        <div class="grid grid-cols-2 gap-3 text-sm">
...
window.editTeacherFromModal = function() {
...
window.saveTeacherEdit = async function() {
    ...
    showLoading();
    try {
        await api.admin.updateTeacher(teacherId, { name, email, subjects, department });
