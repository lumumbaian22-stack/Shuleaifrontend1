...
async function buildReportCardHTML(studentId) {
    ...
    <style>
      body{font-family:Arial,sans-serif;margin:0;padding:32px;color:#172033;background:#fff}.report{max-width:900px;margin:auto;border:1px solid #d9e2ef;border-radius:18px;padding:28px}.header{text-align:center;border-bottom:3px solid #0b5ed7;padding-bottom:18px;margin-bottom:22px}.header h1{margin:0;color:#083A85}.muted{color:#64748b}.student{display:flex;gap:18px;align-items:center;background:#f8fafc;border-radius:14px;padding:16px;margin-bottom:20px}.photo{height:88px;width:88px;border-radius:50%;object-fit:cover;border:3px solid #11B5B1}.avatar{height:88px;width:88px;border-radius:50%;display:flex;align-items:center;justify-content:center;background:#083A85;color:#fff;font-size:28px;font-weight:700}table{width:100%;border-collapse:collapse;margin-top:12px}th{background:#083A85;color:#fff;text-align:left;padding:10px}td{border-bottom:1px solid #e2e8f0;padding:10px}.summary{display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin:18px 0}.card{border:1px solid #e2e8f0;border-radius:12px;padding:12px;background:#f8fafc}.signatures{display:grid;grid-template-columns:repeat(3,1fr);gap:22px;margin-top:35px}.sig{border-top:1px solid #334155;padding-top:8px;font-size:12px}.footer{text-align:center;margin-top:24px;font-size:12px;color:#64748b}@media print{body{padding:0}.report{border:0}.no-print{display:none}}
    </style></head><body><div class="report">
      <div class="header"><h1>${escapeHtml(school.schoolName || school.name || 'Shule AI School')}</h1><h2>Official Student Report Card</h2><p class="muted">${escapeHtml(school.curriculum || school.system || 'cbc').toUpperCase()} • Generated ${new Date().toLocaleDateString()}</p></div>
      ...
      <p class="footer">Generated by Shule AI. This report uses published marks only.</p>
    </div></body></html>`;
}
