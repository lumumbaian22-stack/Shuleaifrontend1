...
async function buildReportCardHTML(studentId) {
    ...
    <style>
      ...
      <h3>Academic Performance</h3><table><thead><tr><th>Subject</th><th>Average</th><th>Grade</th></tr></thead><tbody>${subjects.length ? subjects.map(s => `<tr><td>${escapeHtml(s.subject)}</td><td>${s.average}%</td><td><strong>${escapeHtml(s.grade)}</strong></td></tr>`).join('') : (draftPreview ? '<tr><td colspan="3">No marks captured for this preview yet.</td></tr>' : '<tr><td colspan="3">No published marks yet.</td></tr>')}</tbody></table>
