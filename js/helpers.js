...
async function buildReportCardHTML(studentId) {
    ...
    let reportFooter = branding.reportFooter || (draftPreview ? 'Generated by Shule AI. Draft preview uses current saved marks and is not visible to parents/students until published.' : 'Generated by Shule AI. This report uses the official published report snapshot.');
    let subjects = Array.isArray(academic.subjects) ? academic.subjects.slice() : [];
    const assessments = Array.isArray(data.recentAssessments) ? data.recentAssessments : [];
    const hasSubjectScores = subjects.some(s => Number.isFinite(Number(s.average ?? s.score)));
    ...
    const computedOverall = subjects.length ? Math.round(totalScore / subjects.length) : 0;
    if (draftPreview && (!Number(academic.overallAverage) || Number(academic.overallAverage) === 0) && computedOverall) academic.overallAverage = computedOverall;
    const media = (value) => {
