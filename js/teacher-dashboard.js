// teacher-dashboard.js - Complete working version
...
function getTeacherRole() {
  ...
  if (user.teacher && (user.teacher.classTeacher || user.teacher.classId)) return 'class_teacher';
  return 'subject_teacher';
}
function isClassTeacher() { return getTeacherRole() === 'class_teacher'; }
function getTeacherAssignedClass() {
  const user = getCurrentUser();
  if (!user || user.role !== 'teacher') return null;
  if (user.teacher?.classId) return { id: user.teacher.classId, name: user.teacher.className || 'Assigned Class', studentCount: user.teacher.studentCount || 0 };
  if (user.classTeacher) return { id: null, name: user.classTeacher, studentCount: 0 };
  return null;
}
...
async function renderTeacherSection(section) {
  switch(section) {
    ...
    case 'students': return isClassTeacher() ? await renderTeacherStudents() : '<div class="text-center py-12"><i data-lucide="lock"></i><p>Only Class Teachers can manage students.</p></div>';
    ...
    case 'help': return await renderHelpSection();
    case 'profile': return await renderProfileSection();
    default: return await renderTeacherDashboard();
  }
}

// ============ DASHBOARD (real stats, charts, no quick actions inside) ============
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <div class="flex items-center flex-wrap gap-2">
              <h2 class="text-2xl font-bold">Welcome, ${escapeHtml(user?.name)}!</h2>
              <span class="px-2 py-1 bg-blue-100 text-blue-700 text-xs rounded-full">${role === 'class_teacher' ? 'Class Teacher' : 'Subject Teacher'}</span>
            </div>
            <p class="text-muted-foreground mt-1">${role === 'class_teacher' ? 'Manage your class, students, and grades.' : 'Enter marks for your assigned subjects.'}</p>
          ...
          ${isClassTeacher() ? `<button onclick="showCSVUploadModal()" class="px-4 py-2 bg-green-600 text-white rounded-lg flex items-center gap-2"><i data-lucide="upload"></i> Upload Students</button>` : ''}
        </div>
      </div>

      <!-- Stats Grid -->
      <div class="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <div class="rounded-xl border bg-card p-6"><div class="flex justify-between"><div><p class="text-sm text-muted-foreground">My Students</p><h3 class="text-2xl font-bold">${stats.studentCount}</h3></div><div class="h-12 w-12 rounded-lg bg-blue-100 flex items-center justify-center"><i data-lucide="users" class="text-blue-600"></i></div></div></div>
        <div class="rounded-xl border bg-card p-6"><div class="flex justify-between"><div><p class="text-sm text-muted-foreground">Class Average</p><h3 class="text-2xl font-bold">${stats.classAverage}%</h3></div><div class="h-12 w-12 rounded-lg bg-violet-100"><i data-lucide="trending-up" class="text-violet-600"></i></div></div></div>
        <div class="rounded-xl border bg-card p-6"><div class="flex justify-between"><div><p class="text-sm text-muted-foreground">Attendance Today</p><h3 class="text-2xl font-bold">${stats.attendanceToday}</h3></div><div class="h-12 w-12 rounded-lg bg-amber-100"><i data-lucide="calendar-check" class="text-amber-600"></i></div></div></div>
        <div class="rounded-xl border bg-card p-6"><div class="flex justify-between"><div><p class="text-sm text-muted-foreground">Pending Tasks</p><h3 class="text-2xl font-bold">${stats.pendingTasks}</h3></div><div class="h-12 w-12 rounded-lg bg-red-100"><i data-lucide="check-square" class="text-red-600"></i></div></div></div>
      </div>

      <!-- Charts -->
      <div class="grid gap-4 lg:grid-cols-2">
        <div class="rounded-xl border bg-card p-6"><h3 class="font-semibold mb-4">Subject Performance</h3><div class="chart-container h-64"><canvas id="teacher-performanceChart"></canvas></div></div>
        <div class="rounded-xl border bg-card p-6"><h3 class="font-semibold mb-4">Attendance Trend (Last 7 days)</h3><div class="chart-container h-64"><canvas id="teacher-gradeChart"></canvas></div></div>
      </div>

      <!-- Parent Messages Inbox -->
      <div class="rounded-xl border bg-card p-6">
        <div class="flex justify-between items-center mb-4"><h3 class="font-semibold">Parent Messages</h3><span id="teacher-message-count-badge" class="bg-red-100 text-red-700 px-2 py-1 rounded-full text-xs">0</span></div>
        <div id="teacher-messages-list" class="space-y-2 max-h-96 overflow-y-auto"><div class="text-center text-muted-foreground py-8">Loading messages...</div></div>
        <button onclick="loadTeacherMessages()" class="mt-4 w-full py-2 text-sm border rounded-lg">Refresh Messages</button>
      </div>
    </div>
  `;

  setTimeout(() => {
    if (performanceData.subjectAverages.length) {
      const perfCtx = document.getElementById('teacher-performanceChart');
      if (perfCtx && window.teacherPerfChart) window.teacherPerfChart.destroy();
      if (perfCtx) window.teacherPerfChart = new Chart(perfCtx, { type: 'line', data: { labels: performanceData.subjectAverages.map(s=>s.subject), datasets: [{ label: 'Average Score (%)', data: performanceData.subjectAverages.map(s=>s.average), borderColor: '#8b5cf6', tension: 0.4 }] }, options: { responsive: true, maintainAspectRatio: false, scales: { y: { beginAtZero: true, max: 100 } } } });
    }
    if (performanceData.attendanceTrend.length) {
      const gradeCtx = document.getElementById('teacher-gradeChart');
      if (gradeCtx && window.teacherGradeChart) window.teacherGradeChart.destroy();
      if (gradeCtx) window.teacherGradeChart = new Chart(gradeCtx, { type: 'bar', data: { labels: performanceData.attendanceTrend.map(a=>moment(a.date).format('MMM D')), datasets: [{ label: 'Attendance Rate (%)', data: performanceData.attendanceTrend.map(a=>a.rate), backgroundColor: '#3b82f6', borderRadius: 6 }] }, options: { responsive: true, maintainAspectRatio: false, scales: { y: { beginAtZero: true, max: 100 } } } });
    }
    loadTodayDuty();
    loadTeacherMessages();
  }, 100);
...
async function loadTeacherMessages() {
  ...
  container.innerHTML = convos.map(c => {
    ...
    return `<div class="p-3 border rounded-lg cursor-pointer" onclick="openParentConversation('${c.userId}')"><div class="flex justify-between"><div><p class="font-medium">${c.userName}</p><p class="text-xs">${c.studentName?`about ${c.studentName}`:''}</p><p class="text-sm mt-1">${c.lastMessage?.substring(0,50)}</p></div><div class="text-right"><p class="text-xs">${timeAgo(c.lastMessageTime)}</p>${c.unreadCount?`<span class="bg-red-500 text-white text-xs rounded-full px-2 py-1">${c.unreadCount}</span>`:''}</div></div></div>`;
  }).join('');
  if (badge) badge.textContent = totalUnread;
}
async function openParentConversation(parentId) {
  const messages = await api.teacher.getMessages(parentId);
...
// ============ DUTY & DUTY PREFERENCES ============
async function renderTeacherDuty() {
  const today = await loadTodayDuty();
  const weekly = await loadWeeklyDuty();
  const user = getCurrentUser();
  return `
    <div class="space-y-6"><h2 class="text-2xl font-bold">My Duty Schedule</h2>
    <div class="grid gap-4 md:grid-cols-2">
      <div class="rounded-xl border bg-card p-6"><h3 class="font-semibold mb-4">This Week's Duty</h3><div class="space-y-3">${weekly?.filter(day => day.duties?.some(d => d.teacherId === user?.id)).map(day => day.duties?.filter(d => d.teacherId === user?.id).map(duty => `<div class="flex justify-between p-3 bg-muted/30 rounded"><div><p class="font-medium">${day.dayName}</p><p class="text-sm">${duty.area}</p></div><span>${duty.timeSlot?.start} - ${duty.timeSlot?.end}</span></div>`).join('')).join('') || '<p class="text-center text-muted-foreground">No duty this week</p>'}</div></div>
      <div class="rounded-xl border bg-card p-6"><h3 class="font-semibold mb-4">Request Duty Swap</h3><div class="space-y-3"><input type="date" id="swap-date" class="w-full rounded-lg border p-2"><textarea id="swap-reason" rows="2" class="w-full rounded-lg border p-2" placeholder="Reason"></textarea><button onclick="requestDutySwap()" class="w-full bg-primary text-white py-2 rounded-lg">Submit Request</button></div></div>
    </div></div>
  `;
}
async function loadTodayDuty() {
  try { const res = await api.duty.getTodayDuty(); return res.data; } catch(e) { return null; }
}
async function loadWeeklyDuty() {
  try { const res = await api.duty.getWeeklyDuty(); return res.data; } catch(e) { return []; }
}
async function requestDutySwap() {
  ...
  if (!date || !reason) return showToast('Fill all fields', 'error');
  try {
...
function renderTeacherDutyPreferences() {
  return `<div class="space-y-6"><h2 class="text-2xl font-bold">Duty Preferences</h2><div class="rounded-xl border bg-card p-6 max-w-2xl mx-auto"><div class="space-y-4"><div><label>Preferred Days</label><div class="flex flex-wrap gap-3" id="pref-days">${['Monday','Tuesday','Wednesday','Thursday','Friday'].map(d => `<label><input type="checkbox" value="${d.toLowerCase()}" class="pref-day"> ${d}</label>`).join('')}</div></div><div><label>Preferred Areas</label><div class="flex flex-wrap gap-3" id="pref-areas">${['morning','lunch','afternoon','whole_day'].map(a => `<label><input type="checkbox" value="${a}" class="pref-area"> ${a}</label>`).join('')}</div></div><div><label>Max Duties Per Week</label><input type="number" id="max-duties" value="3" min="1" max="5" class="w-full rounded-lg border p-2"></div><div><label>Blackout Dates</label><div class="flex gap-2"><input type="date" id="blackout-date" class="flex-1 rounded-lg border p-2"><button onclick="addBlackoutDate()" class="px-3 py-2 bg-primary text-white rounded-lg">Add</button></div><div id="blackout-dates-list" class="mt-2"></div></div><button onclick="saveDutyPreferences()" class="w-full bg-primary text-white py-2 rounded-lg">Save Preferences</button></div></div></div>`;
}
window.addBlackoutDate = function() {
  ...
  div.className = 'flex justify-between items-center p-2 bg-muted/30 rounded mb-1';
  div.innerHTML = `<span>${new Date(date).toLocaleDateString()}</span><button onclick="this.parentElement.remove()" class="text-red-600"><i data-lucide="x"></i></button>`;
...
async function renderTeacherTasks() {
  let tasks = [];
  try { const res = await api.tasks.getTasks(); tasks = res.data || []; } catch(e) {}
...
async function renderHelpSection() {
  let articles = [];
  try { const res = await api.help.getArticles('teacher'); articles = res.data || []; } catch(e) {}
  return `<div class="space-y-6 max-w-4xl mx-auto"><h2 class="text-2xl font-bold">Help Center</h2><div class="grid gap-4">${articles.map(a => `<div class="rounded-xl border bg-card p-6 cursor-pointer" onclick="alert('${a.title}\n\n${a.content}')"><h3 class="font-semibold">${escapeHtml(a.title)}</h3><p class="text-muted-foreground">${escapeHtml(a.content.substring(0,150))}...</p></div>`).join('')}</div></div>`;
}
async function renderProfileSection() {
  const user = getCurrentUser();
  return `<div class="space-y-6 max-w-4xl mx-auto"><div class="rounded-xl bg-gradient-to-r from-blue-600 to-purple-600 p-8 text-white"><div class="flex items-center gap-6"><div class="h-24 w-24 rounded-full bg-white/20 flex items-center justify-center text-4xl font-bold">${getInitials(user.name)}</div><div><h2 class="text-3xl font-bold">${user.name}</h2><p class="capitalize">${user.role}</p></div></div></div>
  <div class="grid gap-4 md:grid-cols-3"><div class="rounded-xl border bg-card p-4"><p class="text-sm text-muted-foreground">Member Since</p><p>${formatDate(user.createdAt)}</p></div><div class="rounded-xl border bg-card p-4"><p class="text-sm text-muted-foreground">Last Login</p><p>${user.lastLogin ? timeAgo(user.lastLogin) : 'N/A'}</p></div><div class="rounded-xl border bg-card p-4"><p class="text-sm text-muted-foreground">Status</p><p class="text-green-600">Active</p></div></div>
  <div class="rounded-xl border bg-card p-6"><h3 class="font-semibold mb-4">Update Profile</h3><form onsubmit="updateProfile(event)" class="space-y-4"><div class="grid md:grid-cols-2 gap-4"><input type="text" name="name" value="${user.name}" class="rounded-lg border p-2"><input type="email" name="email" value="${user.email||''}" class="rounded-lg border p-2"></div><input type="tel" name="phone" value="${user.phone||''}" class="w-full rounded-lg border p-2"><button type="submit" class="px-4 py-2 bg-primary text-white rounded-lg">Update</button></form></div>
  <div class="rounded-xl border bg-card p-6"><h3 class="font-semibold mb-4">Change Password</h3><form onsubmit="updatePassword(event)" class="space-y-4"><input type="password" id="current-password" placeholder="Current Password" class="w-full rounded-lg border p-2"><input type="password" id="new-password" placeholder="New Password" class="w-full rounded-lg border p-2"><input type="password" id="confirm-password" placeholder="Confirm" class="w-full rounded-lg border p-2"><button type="submit" class="px-4 py-2 bg-primary text-white rounded-lg">Update</button></form></div>
  <div class="rounded-xl border bg-card p-6"><h3 class="font-semibold mb-4">Account Actions</h3><div class="flex gap-3"><button onclick="downloadMyData()" class="px-4 py-2 border rounded-lg">Download Data</button><button onclick="deactivateAccount()" class="px-4 py-2 border border-red-300 text-red-600 rounded-lg">Deactivate</button></div></div></div>`;
}
async function updateProfile(e) {
  e.preventDefault(); const data = { name: e.target.name.value, email: e.target.email.value, phone: e.target.phone.value };
  try { await api.user.updateProfile(data); const user = getCurrentUser(); user.name=data.name; user.email=data.email; user.phone=data.phone; localStorage.setItem('user',JSON.stringify(user)); showToast('Profile updated','success'); await showDashboardSection('profile'); } catch(err){ showToast(err.message,'error'); }
}
async function updatePassword(e) {
  e.preventDefault(); const current=document.getElementById('current-password').value, newPwd=document.getElementById('new-password').value, confirm=document.getElementById('confirm-password').value;
  if(newPwd!==confirm) return showToast('Passwords do not match','error');
  if(newPwd.length<8) return showToast('Minimum 8 characters','error');
  try { await api.auth.changePassword(current,newPwd); showToast('Password changed','success'); document.getElementById('current-password').value=''; document.getElementById('new-password').value=''; document.getElementById('confirm-password').value=''; } catch(err){ showToast(err.message,'error'); }
}
async function downloadMyData() {
  try { const res = await api.user.exportMyData(); const blob = new Blob([JSON.stringify(res.data,null,2)],{type:'application/json'}); const url=URL.createObjectURL(blob); const a=document.createElement('a'); a.href=url; a.download=`shuleai_data.json`; a.click(); URL.revokeObjectURL(url); showToast('Data exported','success'); } catch(err){ showToast(err.message,'error'); }
}
async function deactivateAccount() {
  if(!confirm('Deactivate your account?')) return; const reason=prompt('Reason (optional)');
  try { await api.user.deactivateAccount(reason); showToast('Account deactivated. Logging out...','info'); setTimeout(()=>logout(),2000); } catch(err){ showToast(err.message,'error'); }
}

// ============ OTHER HELPER FUNCTIONS (students, attendance, marks) ==========
async function renderTeacherStudents() {
  const students = await api.teacher.getMyStudents();
  return `<div class="overflow-x-auto"><table class="w-full"><thead><tr><th>Student</th><th>ELIMUID</th><th>Grade</th></tr></thead><tbody>${students.data.map(s=>`<tr><td>${escapeHtml(s.User?.name)}</td><td>${s.elimuid}</td><td>${s.grade}</td></tr>`).join('')}</tbody></table></div>`;
}
async function renderTeacherAttendance() {
  const students = await api.teacher.getMyStudents();
  if (!students.data.length) return '<div class="text-center py-12">No students</div>';
  return `<div class="space-y-6"><h2 class="text-2xl font-bold">Take Attendance</h2><div class="rounded-xl border bg-card overflow-hidden"><div class="p-4 border-b flex justify-end"><button onclick="saveAttendance()" class="px-4 py-2 bg-primary text-white rounded-lg">Save Attendance</button></div><div class="overflow-x-auto"><table class="w-full"><thead><tr><th>Student</th><th>ELIMUID</th><th>Status</th><th>Notes</th></tr></thead><tbody>${students.data.map(s=>`<tr><td>${escapeHtml(s.User?.name)}</td><td>${s.elimuid}</td><td><select class="attendance-status border rounded p-1"><option value="present">Present</option><option value="absent">Absent</option><option value="late">Late</option></select></td><td><input type="text" class="attendance-note border rounded p-1" placeholder="Note"></td></tr>`).join('')}</tbody></table></div></div></div>`;
}
...
async function renderTeacherMarksEntry() {
  ...
  try {
    ...
    if (res.data?.classTeacher) assignments.push({ type: 'class', id: res.data.classTeacher.id, name: res.data.classTeacher.name, subject: 'All Subjects' });
    for (const sub of (res.data?.subjects || [])) assignments.push({ type: 'subject', id: sub.classId, name: sub.className, subject: sub.subject });
  } catch(e) {}
  ...
  for (const a of assignments) {
    html += `<div class="rounded-xl border bg-card p-5 cursor-pointer hover:shadow-md" onclick="openMarksEntry('${a.subject}', '${a.id}', '${a.name}')"><div class="flex items-center gap-3 mb-3"><div class="h-12 w-12 rounded-xl bg-primary/10 flex items-center justify-center"><i data-lucide="book" class="text-primary"></i></div><div><p class="font-semibold">${escapeHtml(a.name)}</p><p class="text-sm text-muted-foreground">${escapeHtml(a.subject)}</p></div></div><button class="w-full py-2 text-sm bg-primary text-white rounded-lg">Enter Marks</button></div>`;
  }
  html += `</div></div>`;
  return html;
}
