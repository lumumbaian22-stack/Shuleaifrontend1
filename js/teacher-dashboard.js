...
// V6 teacher chat avatar helpers
function renderTeacherWhatsAppMessage(message) {
    const currentUser = getCurrentUser && getCurrentUser();
    return ShuleChatUI.messageBubble(message, currentUser?.id);
}

function renderTeacherChatUser(user, onclick = '') {
    return ShuleChatUI.userRow(user, onclick);
}
