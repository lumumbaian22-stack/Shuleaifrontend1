...
(function(){
  ...
  w.getSafeSchoolLogo=function(school){
    ...
    if(!logo || /undefined|null|\/uploads\//i.test(String(logo))) return 'assets/logo.png?v=1510';
    return logo;
  };
...
(function(){
  const w=window;
  const esc=(v)=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
  function getLogo(school){ try{return w.getSafeSchoolLogo? w.getSafeSchoolLogo(school||{}):'assets/logo.png?v=1510';}catch(_){return 'assets/logo.png?v=1510';} }
  w.renderFinalReportCardMockup=function(data={}){
