// Shule AI v146 — visible, role-owned lifecycle screens with simplified workflows.
...
(function(){
  ...
  function reportCardDesignPanel(role, rows){
    const latest = (rows || []).find(r => r.isCurrent) || (rows || [])[0];
    const title = role === 'admin' ? 'Final Report Card Template & Published Reports' : role === 'teacher' ? 'Final Report Card Preview' : 'Final Report Card';
    const sub = role === 'admin'
      ? 'Admin chooses the visible assessment columns in Report Card Settings. The published PDF uses the A4 design with logo, watermark, selected tests, comments and signatures.'
      : role === 'teacher'
        ? 'Class teachers preview the A4 design before publishing. Published reports are what parents and students see.'
        : 'Parents and students see the same official A4 report card after the school publishes it.';
    const sample = window.renderFinalReportCardMockup ? window.renderFinalReportCardMockup({school:{name:'School Report Preview'},student:{name:'Learner Preview',elimuid:'ELIMU-ID',className:'Class / Stream'},term:'Term',year:new Date().getFullYear(),subjects:[{subject:'Mathematics',components:[{score:'—'},{score:'—'},{score:'—'},{score:'—'},{score:'—'}],average:'—',grade:'—',remark:'Selected tests appear here'}]}) : '';
    return `<section class="rounded-2xl border bg-card overflow-hidden"><div class="p-5 border-b flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3"><div><p class="text-xs uppercase tracking-wide text-muted-foreground">A4 report-card design</p><h3 class="text-xl font-bold">${esc(title)}</h3><p class="text-sm text-muted-foreground mt-1">${esc(sub)}</p></div><div class="flex gap-2 flex-wrap">${role==='admin'?`<button onclick="showDashboardSection('report-settings')" class="px-4 py-2 rounded-lg bg-primary text-primary-foreground">Choose Assessment Columns</button>`:''}${latest?`<button onclick="openReportSnapshotPdf(${Number(latest.id)})" class="px-4 py-2 rounded-lg border">Open Latest Published PDF</button>`:''}</div></div><div class="p-4 overflow-x-auto"><div style="transform:scale(.55);transform-origin:top left;width:210mm;min-height:160mm;" class="pointer-events-none opacity-95">${sample}</div></div></section>`;
  }
