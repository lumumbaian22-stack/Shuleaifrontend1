...
function updateUserInfo() {
    ...
    const photo = user.profileImage || user.profilePicture || user.avatar || user.photo || '';
    const photoUrl = photo ? (typeof safeResolveMediaUrl === 'function' ? safeResolveMediaUrl(photo) : (typeof resolveMediaUrl === 'function' ? resolveMediaUrl(photo) : photo)) : '';
    ...
    if (avatarWrap) {
        if (photoUrl) {
            avatarWrap.classList.remove('bg-gradient-to-br', 'from-blue-500', 'to-violet-600');
            avatarWrap.innerHTML = `<img src="${photoUrl}" class="h-full w-full rounded-full object-cover user-avatar" data-current-user-avatar data-profile-full="${photoUrl}" data-profile-name="${escapeHtml(name)}" alt="${escapeHtml(name)}">`;
        } else {
            avatarWrap.classList.add('bg-gradient-to-br', 'from-blue-500', 'to-violet-600', 'flex', 'items-center', 'justify-center');
